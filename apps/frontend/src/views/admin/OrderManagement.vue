        ...
        <div class="flex justify-between h-16">
          <div class="flex items-center">
            ...
            <div class="ml-10 flex items-center space-x-4">
              ...
              <span class="text-gray-900 font-medium">订单管理</span>
            </div>
          </div>
          <div class="flex items-center">
      ...
      <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-4">
          <button class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors duration-200 flex items-center">
            <i class="fa fa-file-excel-o mr-2"></i> 导出订单
          </button>
      ...
      <div class="bg-white p-4 rounded-lg shadow mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input type="text" v-model="userId" placeholder="输入用户ID" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
          </div>
          <div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="order in orders" :key="order.id">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span :class="['px-2 inline-flex text-xs leading-5 font-semibold rounded-full', order.type === 'food' ? 'bg-yellow-100 text-yellow-800' : order.type === 'ride' ? 'bg-blue-100 text-blue-800' : order.type === 'errand' ? 'bg-green-100 text-green-800' : 'bg-purple-100 text-purple-800']">
                  {{ order.type === 'food' ? '美食订单' : order.type === 'ride' ? '出行订单' : order.type === 'errand' ? '跑腿订单' : '商城订单' }}
                </span>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span :class="['px-2 inline-flex text-xs leading-5 font-semibold rounded-full', order.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : order.status === 'processing' ? 'bg-blue-100 text-blue-800' : order.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800']">
                  {{ order.status === 'pending' ? '待处理' : order.status === 'processing' ? '处理中' : order.status === 'completed' ? '已完成' : '已取消' }}
                </span>
              </td>
...
onMounted(() => {
  // 这里可以从API获取真实数据
  console.log('Order management page mounted');
});
</script>
